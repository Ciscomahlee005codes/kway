...
const formatTime = (timestamp) => {
  ...
  const isToday =
    storyTime.toDateString() === now.toDateString();

  const yesterday = new Date();
  yesterday.setDate(now.getDate() - 1);

  const isYesterday =
    storyTime.toDateString() === yesterday.toDateString();

  const time = storyTime.toLocaleTimeString("en-NG", {
    hour: "numeric",
    minute: "2-digit",
    hour12: true
  });
  ...
  return storyTime.toLocaleDateString("en-NG", {
    day: "numeric",
    month: "short"
  }) + ` ${time}`;
